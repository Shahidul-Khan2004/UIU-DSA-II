...
pair<pair<int, int>, pair<int, int>> max_min (vector<int> &arr, int beginning, int end) {
    ...
    int secondMaximum = max(min(res_left.first.first, res_right.first.first), max(res_left.second.first, res_right.second.first));
    int minimum = min(res_left.first.second, res_right.first.second);
    int secondMinimum = min(max(res_left.first.second, res_right.first.second), min(res_left.second.second, res_right.second.second));
